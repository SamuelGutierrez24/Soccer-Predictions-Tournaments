...
export default function Ranking({ pollaId }: RankingProps) {
  ...
  const fetchRankingData = async () => {
    try {
      ...
      const posRes = await userScoresPollaApi.getUserPositionInPolla(pollaId, userId, token);
      console.log('User position data:', posRes);
      const position = posRes.position;
      const points = posRes.score;
      const userInfo = await authApi.getMyProfile(token);
  ...
  return (
    <div className="text-center">
      ...
      {userRanking && (
        <div className="bg-white shadow-lg rounded-xl py-6 px-8 mt-10 max-w-md mx-auto text-center">
          ...
          <div className="text-lg font-semibold">{userRanking.nickname}</div>
